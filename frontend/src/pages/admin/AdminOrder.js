...
const AdminOrder = () => {
  ...
  const allOrders = async () => {
    try {
      const { data } = await axios.get(
        "https://busy-pink-chinchilla-suit.cyclic.app/api/product/all-order",
        config
      );
  ...
  const handleChange=async(status,id)=>{
    try {
      const {data}=await axios.put(`https://busy-pink-chinchilla-suit.cyclic.app/api/product/update-order/${id}`,{status},config)
      allOrders();
                ...
                {order.products.map((product) => (
                  ...
                  >
                    <div className="col-md-4 flex-row">
                      <img
                        src={`https://busy-pink-chinchilla-suit.cyclic.app/api/product/product-photo/${product._id}`}
                    ...
                    <div className="col-md-6" style={{ margin: "20px" }}>
                      ...
                      <h6 className="card-text">Price: {product.price*product.quantity}</h6>
                    </div>
                  </div>
                ))}
              </div>
